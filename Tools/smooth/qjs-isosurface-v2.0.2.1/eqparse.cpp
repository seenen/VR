...
string quaternion_julia_set_equation_parser::emit_glsl_shader_code(void)
{
	string code;

	code += "#version 110\n";
	code += "#extension GL_ARB_draw_buffers : enable\n";
	code += "\n";
	code += "uniform sampler2D z_xyz;\n";
	code += "uniform float z_w;\n";
	code += "uniform vec4 c;\n";
	code += "uniform int max_iterations;\n";
	code += "uniform float threshold;\n";

	code += "\n";
	code += q_math.emit_glsl_function_definitions();
	code += "\n";
	code += emit_glsl_execution_stack_shader_code();
	code += "\n";

	code += "float iterate(vec4 z)\n";
	code += "{\n";
	code += "    const float threshold_sq = threshold*threshold;\n";
	code += "\n";
	...
	code += "void main(void)\n";
	code += "{\n";
	code += "    vec4 z = vec4(texture2D(z_xyz, gl_TexCoord[0].st).xyz, z_w);\n";
	code += "\n";
	code += "    gl_FragData[0].rgba = iterate(z);\n";
	code += "}\n";

	return code;
}

string quaternion_julia_set_equation_parser::emit_glsl_execution_stack_shader_code(void)
{
	string code;
