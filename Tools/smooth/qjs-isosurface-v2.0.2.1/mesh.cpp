...
void indexed_mesh::fix_cracks(void)
{
	...
	// Make a copy of the set into a vector because this O(n^2) algorithm will
	...
	for(size_t i = 0; i < problem_edges_vec.size(); i++)
	{
		...
		// If the indices aren't already the same, then merge them.
...
template<typename T> void indexed_mesh::eliminate_vector_duplicates(vector<T> &v)
{
	...
	// Probably should use copy with back inserter, but whatever...
	for(typename set<T>::const_iterator ci = s.begin(); ci != s.end(); ci++)
		v.push_back(*ci);
}
